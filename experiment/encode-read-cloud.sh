...
echo 'for i in {0..9}; do rados -p claypool get NYAN$i outfile && echo read $i; done' >> help.sh
sudo cephadm shell -m help.sh:/help.sh -- bash help.sh
...
sshpass -p Cloud2022 ssh root@$PRIMARY_HOST "journalctl -u ceph-$CEPH_ID@osd.$PRIMARY | grep ErasureCodeMyClay > $MODE-encode-read.log"
sshpass -p Cloud2022 scp root@$PRIMARY_HOST:$MODE-encode-read.log .  # 将PRIMARY_HOST上的日志传到本机(machine001)
